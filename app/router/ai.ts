...
// const MODEL_ID = "liquid/lfm-2.5-2.6b:free";
...
export const generateThreadSummary = base
  ...
  .handler(async ({ input, context, errors }) => {
    ...
    const system = [
      ...
      "If the context is insufficient, return a single-sentence summary and omit the bullet lists.",
    ].join("\n");
...
export const generateCompose = base
  ...
  .handler(async ({ input }) => {
    ...
    // Focused system prompt for deterministics rewrite (non-conversational)
    const system = [
      "You are an expert rewrting assistant. You are not a chatbot.",
